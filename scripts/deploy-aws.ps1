...
function Invoke-Preflight($d) {
  ...
  foreach ($k in @('brevo.password','sentinel_hub.client_secret','keycloak.admin_password','keycloak.client_secret','keycloak.db_password','jwt.signing_key','postgres.superuser_password')) {
    $parts = $k.Split('.')
...
function Invoke-SeedSecrets($d) {
  ...
  $map = @{
    ...
    "agripulse/$env/keycloak-db-password"        = $d.keycloak.db_password
