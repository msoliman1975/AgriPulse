...
> Paste the entire content below the `---` line into Claude Code as the first user message of a fresh session. Make sure Claude Code has the repository `msoliman1975/AgriPulse` available locally and that you have placed `docs/ARCHITECTURE.md` and `docs/data_model.md` in the repository's `docs/` directory before starting.

---

# Mission

You are building the foundation of AgriPulse, a multi-tenant satellite-driven farm management SaaS for Egypt. The repository is `msoliman1975/AgriPulse`. The repository is empty except for `docs/ARCHITECTURE.md` and `docs/data_model.md`. Your job is to deliver Prompt 1 â€” Foundation â€” exactly as scoped below.
...
2. **Do not invent architecture.** Every architectural choice â€” language, framework, library, pattern â€” is in `ARCHITECTURE.md`. If a choice you need to make isn't there, ask.
...
Reproduce here for clarity. Authoritative reference is `prompts/roadmap.md` Â§ Prompt 1.

## Repository structure
Set up the monorepo exactly as described in `ARCHITECTURE.md` Â§ 4. Create empty placeholder folders for modules you are not yet implementing (so the import-linter contract can be defined for the full module set up front).
...
- RBAC:
  - `app/shared/rbac/capabilities.yaml` â€” list of all capabilities the platform recognizes (use the data model + module list to derive a complete-enough first version; mark "stub" capabilities for modules not yet built).
  - `app/shared/rbac/role_capabilities.yaml` â€” eight roles â†’ capability lists.
  - `app/shared/rbac/check.py` â€” `requires_capability("alert.acknowledge", farm_id=...)` as a FastAPI dependency factory and a callable for non-route checks. Resolves the three-layer order: PlatformRole â†’ TenantRole â†’ FarmScope.
- In-process event bus:
  - `app/shared/eventbus/bus.py` â€” supports sync subscribers (run inline) and async subscribers (dispatched as Celery tasks).
  - Subscribers register at app startup via decorators or an explicit registry.
- Celery:
  - Two worker entrypoints: `workers/light/main.py`, `workers/heavy/main.py` â€” same codebase, different `Celery(...)` instances bound to different queues.
  - `workers/beat/main.py` for scheduling. No real schedules yet â€” just the scaffold.
- Observability:
...
- Modules implemented in this prompt:
  - `tenancy`: full minimum â€” Tenant, TenantSubscription, TenantSettings tables; `POST /api/v1/admin/tenants` endpoint (PlatformAdmin only) that creates the tenant, runs the tenant-schema bootstrap (creates `tenant_<uuid>` schema, applies tenant migrations to it).
  - `iam`: full minimum â€” User, UserPreferences, TenantMembership, TenantRoleAssignment, FarmScope, PlatformRoleAssignment tables; `GET /api/v1/me` returns user profile + preferences + memberships + scopes.
  - `audit`: minimal â€” `audit_events` hypertable; `audit.record(event)` interface used by tenant creation. No frontend yet.
...
- Migrations:
  - `backend/migrations/public/` â€” Alembic init for shared schema.
  - `backend/migrations/tenant/` â€” separate Alembic env for the per-tenant schema; a runner script `scripts/migrate_tenants.py` that loops `public.tenants` and applies tenant migrations to each schema with checkpointing.
- import-linter config:
  - Layered architecture contract: shared â†’ modules cannot import each other's internals.
  - One contract per module pair (forbidden cross-module-internal imports). The full set of modules is in scope here even though most are stubs â€” establish the contracts now so they cannot be violated as modules fill in.
- Tests:
...
- OIDC client â€” use `oidc-client-ts` or `react-oidc-context` against Keycloak. Authorization Code flow with PKCE. Tokens stored in memory (not localStorage); refresh tokens via silent renew.
...
- App shell:
  - Header with logo, language toggle (en/ar â€” toggles `dir` on `<html>`), unit toggle (feddan/acre), user menu.
  - Side nav placeholder (modules will fill it later).
  - Routing via `react-router-dom` v6.
- Pages:
  - `/login` â€” redirects to Keycloak.
  - `/me` â€” fetches `/api/v1/me`, displays user info, language preference, unit preference, list of tenant memberships and farm scopes.
  - `/` â€” placeholder dashboard saying "Welcome â€” features coming soon."
...
- No real data integration yet â€” just verify the deployment is reachable from the cluster ingress and returns 200 on `/health`.
...
- **NGINX Ingress Controller** + cert-manager + a wildcard cert for `*.dev.agripulse.local` (or whatever placeholder domain you use for dev).
...
- **Workflows** (`.github/workflows/`):
  - `ci.yml` â€” runs on every PR and push:
  ...
  - `argocd-sync.yml` â€” on merge to `main`, opens a PR in a separate `infra/argocd/` ApplicationSet config to bump image tags. Or use ArgoCD Image Updater â€” your choice; document it.
  - `dependabot.yml` â€” pip, npm, Docker, GitHub Actions, weekly.
...
- **CODEOWNERS** at `.github/CODEOWNERS` â€” single line owning the whole repo (`* @msoliman1975`); we'll refine as the team grows.
...
- Performance tuning beyond reasonable defaults â€” Prompt 6 handles polish.
...
1. `git push` to `main` triggers CI; all CI jobs pass; container images appear in `ghcr.io/msoliman1975/agripulse/{api,workers,tile-server,frontend}` tagged with the commit SHA.
...
7. A request to a protected endpoint:
   - With no JWT â†’ 401.
   - With a valid JWT but lacking the required capability â†’ 403.
   - With the right JWT and capability â†’ 200.
