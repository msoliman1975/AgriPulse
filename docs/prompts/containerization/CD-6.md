# CD-6 — S3 buckets + IRSA for imagery and Postgres backups
...
## Files to change
- `infra/terraform/s3.tf` — extend. Buckets per env:
  - `agripulse-imagery-{dev,staging,prod}`
  - `agripulse-pg-backup-{dev,staging,prod}`
- `infra/terraform/iam-irsa.tf` (or `iam.tf`) — new IRSA roles:
  - `agripulse-api-irsa-{env}` with imagery bucket RW.
  - `agripulse-cnpg-irsa-{env}` with backup bucket RW.
- `infra/terraform/outputs.tf` — output role ARNs.
- `infra/helm/api/values.yaml` — `serviceAccount.annotations` block referencing the api IRSA ARN (per-env override).
- `infra/helm/shared/values.yaml` — `postgresCluster.serviceAccountAnnotations` for the CNPG-managed SA.
- `infra/argocd/overlays/{dev,staging,production}/values.yaml` — set the role ARN per env.
- `backend/app/shared/storage/s3.py` (if it exists; otherwise the boto3 client wherever it's configured) — confirm it uses default credential chain (no static keys). Don't add new code — verify.

## Tasks
1. S3 buckets: versioning enabled, KMS-encrypted with the existing `aws_kms_key.missionagre`, public access blocked (all four flags), lifecycle rule transitioning `noncurrent_version` objects to STANDARD_IA after 30 days and DEEP_ARCHIVE after 90 (imagery), or expiring backups after 30 days (Postgres backup buckets — matches PITR window from CD-7).
