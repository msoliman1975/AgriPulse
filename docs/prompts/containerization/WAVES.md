# Parallel execution plan for CD-0 … CD-15

Strategy doc: `docs/proposals/app-containerization-strategy.md`. Prompts: `CD-N.md` in this directory.

This file lays out how to **author the 15 PRs in 4–5 wall-clock days instead of ~15**, by running multiple Claude Code sessions concurrently across git worktrees.
...
```
CD-1  CD-2  CD-3  CD-4         ← no deps on anything
CD-5  CD-6  CD-8  CD-11        ← no deps on each other
CD-7  ← CD-6                    (S3 buckets + IRSA)
CD-9  ← CD-6                    (IRSA pattern; soft dep)
CD-10 ← CD-5, CD-9              (hostname + admin secret)
CD-12 ← (none, CI-only)
CD-13 ← CD-7                    (CNPG operator must be backup-enabled)
CD-15 ← (none)
CD-14 ← CD-13                   (Keycloak hostnames in prod overlay)
```
...
| --- | --- | --- | --- | --- |
| **0** | CD-0 | — | Day 1 AM (2h) | Pre-flight file-split refactor. Must land alone. |
| **1** | CD-1, CD-2, CD-3, CD-4 | CD-0 | Day 1 PM → Day 2 | Local + observability. No AWS touched. |
| **2** | CD-5, CD-6, CD-8, CD-11 | CD-0 | Day 3 → Day 4 | Foundational AWS. Author parallel, **serialise `terraform apply` in merge order CD-5 → CD-6 → CD-11 → CD-8**. |
...
| **5** | CD-12, CD-13, CD-15 | CD-10 (soft); CD-13 needs CD-7 | Day 7–8 | Argocd-sync fix + Keycloak prod + cost guardrails. |
| **6** | CD-14 (solo) | CD-13 | Day 9 | Production overlay finalisation + runbooks. |

**Total: ~9 working days vs ~15 serial.** The biggest compression is in Waves 1, 2, and 5 (4×, 4×, 3× respectively).

---

## 3. Execution mechanics — running 4 Claude sessions in parallel
...
git worktree add ../missionagre-cd-1 cd-1
git worktree add ../missionagre-cd-2 cd-2
git worktree add ../missionagre-cd-3 cd-3
git worktree add ../missionagre-cd-4 cd-4
```
...
3. Let them run independently. Each is ~1–3 hours of focused work.
4. As each PR opens, review **in the wave's prescribed merge order** (see §2 notes).
...
6. When the wave is done, prune worktrees: `git worktree remove ../missionagre-cd-N`.

### Tip — running Claude sessions truly in parallel
Each Claude Code session is independent. You can leave one running in a terminal, switch to another worktree's terminal, and launch a second session. They don't share context, which is exactly what you want — each one stays scoped to its single CD-N prompt.
...
| `infra/argocd/overlays/<env>/global.yaml` | CD-12 (tag-bump only) | All — but only the `images.*.tag` lines, edited by CI bot |
| `.github/workflows/ci.yml` | CD-11 then CD-12 | Serial — CD-12 lands after CD-11 in Wave 5 |
| `infra/argocd/appsets/platform.yaml` | CD-5, CD-8 | Same wave — merge CD-5 first |
...
- **Wave 5 ordering**: Land CD-12 first (it's CI-only and tiny). Then CD-13 and CD-15 can land in either order — they touch disjoint files.
...
- **Author wall-clock**: ~3h per PR × 15 PRs = 45h serial; parallelism drops it to ~25h wall-clock across 4–5 sessions.
- **Review wall-clock**: ~30 min per PR × 15 PRs = 7.5h. Cannot be parallelised by one person.
- **Net critical path is review-bound, not author-bound.** Plan for review sessions at end-of-wave: 4 PRs × 30 min = 2h focused review block.
...
- For low-risk PRs (CD-1, CD-12, CD-15) — accept on a skim. Reserve deep review for CD-7, CD-8, CD-10, CD-13.
...
| Reviewer fatigue → PRs pile up | Drop to 2-wide parallelism in subsequent waves; calendar slips by 2–3 days. Still faster than serial. |
