# Application containerization + AWS deployment strategy â€” AgriPulse
...
This document is the source of truth for how AgriPulse is containerized and how it deploys to AWS. Per-PR runnable prompts live in `docs/prompts/containerization/CD-{1..15}.md`.
...
| CI â†’ AWS auth | **GitHub OIDC + scoped IAM role** | No static keys, no rotation burden. |
...
**Containers**
- `backend/Dockerfile` â€” multi-stage with `uv`, `python:3.12-slim-bookworm`, non-root uid 10001, `tini` PID 1. **One image runs api / workers-light / workers-heavy / beat** via `command` override in the Helm chart.
- `frontend/Dockerfile` â€” multi-stage `node:22-alpine` build â†’ `nginxinc/nginx-unprivileged:1.29-alpine` runtime serving the SPA on :8080.
- `tile-server/Dockerfile` â€” wraps `ghcr.io/developmentseed/titiler:0.21.1` with our env defaults and a healthcheck shim.
- `infra/dev/compose.yaml` â€” Postgres + Redis + MinIO + MailHog + Keycloak for local dev.
...
**CI**
- `.github/workflows/ci.yml` â€” pre-commit, backend tests, frontend tests, helm lint/template, terraform fmt/validate, **containers matrix build + push to `ghcr.io/<owner>/agripulse/{api,workers,tile-server,frontend}` tagged with the short SHA on push to `main`** (GHA cache scoped per image).
- `.github/workflows/argocd-sync.yml` â€” auto-opens a PR bumping `image.tag` in `infra/argocd/overlays/dev/values.yaml` after each main push.
...
2. No Alembic migrations Job (ArgoCD PreSync hook) â€” migrations still need to be run manually.
3. No `ServiceMonitor` for `workers`, `tile-server`, `frontend` â€” observability blind spots.
...
5. ArgoCD itself isn't bootstrapped â€” no `helm_release` in Terraform.
6. `argocd-sync.yml` has a known bug: only bumps a single global `image.tag`, missing the tile-server tag.
7. The CNPG `Cluster` template **has the `backup:` block but `backup.enabled` defaults aren't wired** in env overlays â€” needs S3 destination + IRSA role + a `ScheduledBackup` resource.
8. No Karpenter NodePool / EC2NodeClass / controller install.
9. Keycloak prod realm isn't documented; the dev overlay still references `*.agripulse.local` placeholder hostnames.
10. No cost guardrails (Budgets alarm, EBS sweeper, idle-pod Prometheus rules).

---

## 4. PR sequence (CD-1 â€¦ CD-15)
...
Each PR is mergeable + revertable independently. CD-1..CD-7 unblock first dev deploy. CD-8..CD-12 are the cloud-native polish. CD-13..CD-15 graduate dev â†’ prod.
...
| CD-6 | `feat(infra): S3 buckets + IRSA â€” imagery, Postgres backups` | Per-env buckets, lifecycle, KMS encryption. IRSA bindings for the api SA (imagery RW) and CNPG SA (backup RW). |
...
| CD-8 | `feat(infra): Karpenter â€” controller, NodePool, EC2NodeClass` | Replaces the current `eks_managed_node_groups.default` with a minimal baseline; Karpenter scales the rest with spot priority. |
...
| CD-15 | `chore(infra): cost guardrails â€” budget alarm, EBS sweeper, idle-pod alerts` | AWS Budgets at $300/mo with SNS â†’ Slack. Lambda EBS sweeper. Prometheus idle-pod rules. |
...
2. From a workstation with admin creds: `cd infra/terraform && terraform init && terraform apply` â€” creates VPC, EKS, KMS, S3 (imagery + backups), Route 53 zone, OIDC provider, IRSA roles, Karpenter, Secrets Manager entries.
...
7. Verify by visiting `argocd.agripulse.cloud` â†’ 12+ Applications all Healthy + Synced.

Expected time: Â½ day if Terraform applies cleanly; 1â€“2 days if IAM permission boundaries surface (they always do).
...
```
push to main
  â†’ ci.yml builds & pushes images to GHCR (~6 min)
  â†’ argocd-sync.yml opens dev-tag-bump PR (~30 s)
  â†’ merge PR (auto-merge optional)
  â†’ ArgoCD detects change, runs migrations PreSync, rolls Deployments (~3 min)
total: ~10 min from push â†’ dev live
```

**Infra change**
```
PR with infra/terraform/* changes
  â†’ ci.yml runs terraform plan, posts as PR comment
  â†’ reviewer merges
  â†’ infra-apply.yml runs terraform apply with environment approval
total: 2â€“10 min depending on resource
```
...
- **Account split** â€” single account is a known weakness. Migration path: AWS Organization + Control Tower, move prod into its own account, leave dev+staging in the original. Revisit when a paying customer signs an MSA with strong isolation language.
...
- Subscribers fire inside whichever process publishes the event â€” usually a Celery worker.
- **Celery worker containers need the same SMTP env vars as the API container** â€” `send_email()` is called by the worker, not the API.
...
(Originally discovered during the Brevo SMTP rollout on 2026-05-09 â€” the API restart picked up the change but Celery workers had to be restarted separately.)
