# AgriPulse â€” Architecture Decisions

**Repo:** `msoliman1975/AgriPulse`
**Status:** Binding â€” every Claude Code session reads this first.
...
This file is the canonical record of architecture decisions for the AgriPulse platform. It is **binding**: any code, configuration, or library choice that contradicts this file is wrong and must be reverted. New decisions are added here first, code follows.

When in doubt during a Claude Code session: re-read this file. Do not invent new patterns. Do not "improve" decisions. If something looks wrong or under-specified, stop and ask the human â€” do not silently substitute your own judgment.
...
**MVP target:** 1â€“3 paying tenants, 6-week build, single AWS region.
...
- **Tile server:** TiTiler (FastAPI-based COG â†’ XYZ/WMTS)
...
- **Secrets:** External Secrets Operator â†’ AWS Secrets Manager
...
These were considered and rejected â€” do not reintroduce without an explicit decision:

- Django, Flask (we use FastAPI)
- Mapbox GL JS (we use MapLibre GL â€” no proprietary licensing)
...
â”œâ”€â”€ backend/
â”‚   â”œâ”€â”€ app/
â”‚   â”‚   â”œâ”€â”€ modules/         # one folder per module (see Â§ 6)
â”‚   â”‚   â”œâ”€â”€ shared/          # _shared/conditions, _shared/db, _shared/auth_utils
â”‚   â”‚   â”œâ”€â”€ core/            # config, logging, observability, FastAPI app factory
â”‚   â”‚   â””â”€â”€ main.py          # entry point
â”‚   â”œâ”€â”€ workers/
â”‚   â”‚   â”œâ”€â”€ light/           # Celery worker entrypoint for light queue
â”‚   â”‚   â”œâ”€â”€ heavy/           # Celery worker entrypoint for heavy queue
â”‚   â”‚   â””â”€â”€ beat/            # Celery beat schedules
â”‚   â”œâ”€â”€ migrations/
â”‚   â”‚   â”œâ”€â”€ public/          # Alembic migrations for shared schema
â”‚   â”‚   â””â”€â”€ tenant/          # Migrations applied per-tenant by custom runner
â”‚   â”œâ”€â”€ tests/
â”‚   â”‚   â”œâ”€â”€ unit/
â”‚   â”‚   â”œâ”€â”€ integration/
â”‚   â”‚   â””â”€â”€ e2e/
â”‚   â”œâ”€â”€ pyproject.toml       # uv or poetry; we use uv
â”‚   â””â”€â”€ Dockerfile
â”œâ”€â”€ frontend/
â”‚   â”œâ”€â”€ src/
â”‚   â”‚   â”œâ”€â”€ modules/         # mirrors backend modules
â”‚   â”‚   â”œâ”€â”€ shared/
â”‚   â”‚   â”œâ”€â”€ i18n/            # en/ar locale files per namespace
â”‚   â”‚   â””â”€â”€ main.tsx
â”‚   â”œâ”€â”€ tests/
â”‚   â”œâ”€â”€ package.json
â”‚   â””â”€â”€ Dockerfile
â”œâ”€â”€ tile-server/
â”‚   â”œâ”€â”€ Dockerfile           # TiTiler image with our config overrides
â”‚   â””â”€â”€ config/
â”œâ”€â”€ infra/
â”‚   â”œâ”€â”€ helm/
â”‚   â”‚   â”œâ”€â”€ api/
â”‚   â”‚   â”œâ”€â”€ workers/
â”‚   â”‚   â”œâ”€â”€ tile-server/
â”‚   â”‚   â”œâ”€â”€ frontend/
â”‚   â”‚   â””â”€â”€ shared/          # ConfigMaps, Secrets templates, ServiceAccounts
â”‚   â”œâ”€â”€ argocd/              # ApplicationSet manifests
â”‚   â””â”€â”€ terraform/           # AWS bootstrap (VPC, EKS, RDS-if-used, S3, KMS)
â”œâ”€â”€ docs/
â”‚   â”œâ”€â”€ ARCHITECTURE.md      # this file
â”‚   â”œâ”€â”€ data_model.md        # the data model document
â”‚   â”œâ”€â”€ runbooks/
â”‚   â””â”€â”€ decisions/           # ADR-style records for new decisions
â”œâ”€â”€ prompts/                 # Claude Code session prompts
â”œâ”€â”€ .github/
...
## 5. Tenancy model â€” committed

- **Schema-per-tenant** within a single Postgres instance.
- Two schema namespaces:
  - `public` â€” shared catalogs (crops, indices, decision trees, imagery providers, notification templates), tenants table, users table, RBAC tables.
  - `tenant_<uuid>` â€” all tenant-owned data (farms, blocks, indices aggregates, alerts, etc.).
- Tenant resolution from JWT claim only â€” never from URL path or query parameter.
...
- Cross-schema FKs are *logical*, not declarative â€” application enforces them; a periodic consistency-check job logs orphans.

## 6. Module map â€” committed
...
### 6.1 Module boundaries â€” enforced

Each module exposes exactly two things to the outside world:

- `service.py` â€” public Protocol(s) and concrete implementation. Consumers depend on the Protocol; concrete impls wired via dependency injection.
- `events.py` â€” public event types as Pydantic models. Versioned (`FarmCreatedV1`).
...
The single allowed exception is `app/shared/` â€” leaf utilities used everywhere (DB session helpers, auth utilities, the conditions library used by `alerts` and `recommendations`).

### 6.2 Inter-module communication

- **Cross-module commands** (give me X, do Y) â†’ call the target module's service Protocol via DI.
- **Cross-module reactions** (X happened, multiple modules want to know) â†’ publish via in-process event bus; subscribers register handlers at app startup; sync handlers run in-request, async handlers go through Celery.
...
Resolution order: PlatformRole â†’ TenantRole â†’ FarmScope (first match wins).
...
## 8. API conventions â€” committed

- REST, OpenAPI 3.x first.
- All endpoints under `/api/v1/...`.
- No tenant in the URL path â€” always resolved from JWT.
...
## 9. Imagery pipeline â€” committed
...
## 10. Alerts and recommendations â€” committed

### 10.1 Alerts
- **Tier 2 compound rules** (AND/OR groups, time windows, deltas, trends) â€” *not* arbitrary scripted rules.
- **Pull-based evaluation** every 15 minutes by Celery Beat.
- **Lifecycle:** `open â†’ acknowledged â†’ resolved` (or `â†’ snoozed â†’ open`, or `â†’ auto_resolved`).
...
- **Daily per-block evaluation** (not every 15 min â€” recommendations are slower-moving).
...
- **Lifecycle:** `open â†’ applied | dismissed | deferred | expired`.

### 10.3 Shared condition evaluator
- Located at `backend/app/shared/conditions/`.
- Used by both `alerts` and `recommendations` modules â€” never reimplement.
...
## 11. Internationalization â€” committed
...
- **Numerals in Arabic UI:** Western (0â€“9) by default.
...
## 12. Areas and units â€” committed

- **Internal storage:** square meters (`NUMERIC(14, 2)`), always.
- **Display primary:** feddan (1 feddan = 4,200.83 mÂ²).
- **Display secondary:** acre (toggle in user preferences).
- **API contract:** numeric area fields returned in mÂ². Frontend converts to user's preferred unit at the presentation layer.
- **Never store** `area_feddan`, `area_acre`, or `area_ha` columns. No exceptions.

## 13. CI/CD â€” committed
...
- **Artifact registry:** GitHub Container Registry (`ghcr.io/msoliman1975/agripulse/*`).
...
## 14. Observability conventions â€” committed
