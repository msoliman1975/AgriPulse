...
`infra/helm/keycloak/files/missionagre-realm.json`. The shared chart's
...
1. Open `infra/helm/keycloak/files/missionagre-realm.json` on a feature
   branch.
2. Apply the change. Any `secret`-bearing field on a client must be a
   reference to a SM secret, not a literal — see "Sanitizing secrets"
   below.
...
5. **Force a rolling restart** so the import runs:
   ...
   ```bash
   kubectl rollout restart statefulset/missionagre-keycloak -n missionagre
   ```
...
1. `kubectl port-forward -n missionagre svc/missionagre-keycloak 8080:80`
...
4. **Export back to JSON** and commit so the next pod restart doesn't
   ...
   ```bash
   kubectl exec -n missionagre missionagre-keycloak-0 -- \
     /opt/bitnami/keycloak/bin/kc.sh export \
     --realm missionagre --file /tmp/realm.json
   kubectl cp missionagre/missionagre-keycloak-0:/tmp/realm.json \
     infra/helm/keycloak/files/missionagre-realm.json
   ```
...
- **Clients** with a `secret` field (confidential clients) — set the
...
2. Wait `refreshInterval` (1h) or force a sync:
   ```bash
   kubectl annotate externalsecret -n missionagre missionagre-keycloak-admin \
...
- `clients[*].secret` — confidential client secret. Replace with `""`
  or remove; reset via admin UI/API after deploy.
- `users` — never commit users. The runbook for dev seeds them via
  `keycloak-reset.md`.
- `smtpServer.password` — replace with `""`; SMTP creds live in SM at
...
grep -E '"(secret|password)"\s*:\s*"[^"]+"' \
  infra/helm/keycloak/files/missionagre-realm.json && echo "FOUND SECRETS - DO NOT COMMIT"
```
...
1. Scale to zero: `kubectl scale statefulset/missionagre-keycloak --replicas=0 -n missionagre`
...
3. CNPG's `Database` CR will recreate it on the next reconcile (or
   `kubectl annotate database/missionagre-pg-keycloak cnpg.io/reconciliationLoop=now`).
4. Scale Keycloak back up. `--import-realm` re-imports from the
   ConfigMap into the empty database.

This **wipes user data** along with the realm config — for production
