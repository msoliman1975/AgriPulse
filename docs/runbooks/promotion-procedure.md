# Runbook: promotion procedure (dev → staging → prod)

Promotions are gated by human PRs that bump image tags in the per-env
overlay. There is no automated promotion in V1 — that's the feature,
...
|---|---|---|---|
| `overlays/dev/values.yaml`        | `.github/workflows/argocd-sync.yml` on every push to `main` | auto-merge after CI green | n/a — dev is continuous |
| `overlays/staging/values.yaml`    | hand-authored PR                                          | 1 CODEOWNER             | 24–72h before promote   |
| `overlays/production/values.yaml` | hand-authored PR                                          | 2 CODEOWNERS            | n/a — prod is the floor |

`argocd-sync.yml` only touches the dev overlay — staging and prod live
on a strict pull model.

## Dev → staging

1. Pick the dev image SHAs you want to promote — usually the latest
...
5. Watch:
   - `argocd app wait -l missionagre.io/env=staging --health --timeout 600`
...
1. Open a PR titled `chore(prod): bump images to <short-sha>` updating
   ...
   soaked successfully in staging — never re-build for prod.
...
3. Merge.
4. Prod is not auto-sync — follow `docs/runbooks/first-prod-deploy.md`
   from § 4 (the first-time runbook collapses to "sync the apps and
...
2. Land the fix on the hotfix branch with the minimum delta — no
