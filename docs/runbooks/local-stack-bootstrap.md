...
This runbook brings up the full AgriPulse stack on a developer laptop â€”
...
shipped without any of this glue â€” the `dev_bootstrap.py` script is the
...
- For raster ingestion: Sentinel Hub credentials in your password manager
  (entry `agripulse/sentinel-hub-dev`). Optional â€” without them the
...
> **Gotcha** â€” `pydantic-settings` 2.6+ rejects the comma-separated form
...
`SENTINEL_HUB_CLIENT_SECRET` â€” otherwise leave empty and the pipeline
fails closed loudly.

## 4 â€” Public migrations
...
are bootstrapped per tenant in step 6 â€” don't run tenant migrations
manually unless `scripts/migrate_tenants.py` tells you to.

## 5 â€” Dev tenant + user + Keycloak claims
...
> **Keycloak 26 quirk** â€” the realm's user-profile feature drops
...
# Shell 2 â€” light queue (discover_scenes, register_stac_item, audit fan-out)
python -m celery -A workers.light.main worker -Q light -n light@%h --pool=solo --loglevel=INFO

# Shell 3 â€” heavy queue (acquire_scene, compute_indices)
...
> (gunicorn default in our base image); we map host:8001 â†’ container:80
...
1. Open http://localhost:5173 â†’ bounces to Keycloak.
2. Sign in as `dev@agripulse.local` / `dev`.
3. Land on the home page â†’ click **Farms**.
4. Empty list (200, not 403). `[+ New farm]` â†’ draw a small polygon over
   the Nile delta (~31.20Â°E, 30.10Â°N).
5. Add a block inside the farm.
6. Block detail page now shows the three Slice-2 sections:
   - **Imagery** â€” empty until a scene ingests
   - **Trend** â€” Recharts empty-state
   - **Subscriptions** â€” click "Subscribe to Sentinel-2 L2A"
7. Click **Refresh imagery**:
   ...
   - Without creds: a synthetic failed job appears in the audit log
     (`error_message="sentinel_hub_not_configured"`) â€” confirms the
...
| Celery `KeyError: 'imagery.discover_active_subscriptions'` | `_TASK_PACKAGES` pointing at the package, not the submodule                        | Pull main â€” `app.modules.imagery.tasks` is now the right entry              |
...
| New API routes 404 after `--reload` picked up other files | `uvicorn --reload` (watchfiles on Windows) misses *new* module files                | See [backend-stale-routes.md](backend-stale-routes.md) â€” full restart, nuke every `__pycache__`, drop `--reload` |
