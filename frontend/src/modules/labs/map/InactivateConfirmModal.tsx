    ...
    <div className="absolute inset-0 z-30 flex items-center justify-center bg-black/40 px-4">
      ...
      >
        ...
        <p className="mt-1 text-[11px] text-slate-500">
          This is a soft inactivation — history is kept, but new alerts,
          schedules and integrations will stop. You can reactivate later from
          the archive section.
        </p>
          ...
          {previewError ? (
            ...
            <ul className="space-y-0.5">
              {preview.block_count != null ? (
                <li>
                  <strong>{preview.block_count}</strong> block(s) will
                  inactivate
                </li>
              ) : null}
              <li>
                <strong>{preview.alerts_resolved}</strong> open alert(s) will
                resolve
              </li>
              <li>
                <strong>{preview.irrigation_skipped}</strong> future irrigation
                schedule(s) will skip
              </li>
              <li>
                <strong>{preview.plan_activities_skipped}</strong> planned
                activity(ies) will skip
              </li>
              <li>
                <strong>{preview.weather_subs_deactivated}</strong> weather
                subscription(s) will deactivate
              </li>
              <li>
                <strong>{preview.imagery_subs_deactivated}</strong> imagery
                subscription(s) will deactivate
              </li>
            </ul>
          ) : (
        ...
        <label className="mt-3 block text-[11px] font-medium text-slate-700">
          Type <code className="rounded bg-slate-100 px-1">{confirmKeyword}</code>{" "}
          to confirm
          <input
            type="text"
            value={typed}
