...
/**
 * Status helper. Green if a sync happened in the last 6h with zero
 * failures; yellow if no sync in 6–24h; red if any failure in the
 * last 24h or no sync in 24h+. Neutral if no active subscription.
 */
function statusFor(
...
export function IntegrationsHealthPage(): ReactNode {
  ...
  const [tab, setTab] = useState<"farms" | "blocks">("farms");
  const [selectedFarmId, setSelectedFarmId] = useState<string | null>(null);

  const farmsQ = useFarmIntegrationHealth();
  const blocksQ = useBlockIntegrationHealth(
    tab === "blocks" ? selectedFarmId : null,
  );
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <SegmentedControl
        ...
        value={tab}
        onChange={(v) => setTab(v as "farms" | "blocks")}
      />
